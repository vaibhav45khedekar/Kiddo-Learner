...
IMAGE_DIR = r"D:/vvd/images/shapes"
...
class ShapesApp:
    def __init__(self, root, main_menu, show_frame):
        ...
        self.root.title("shapes in daily Life")
       ...
       #self.root.geometry("450x650")
        ...
        self.app_title = create_title_label(root, "Shapes")
        self.app_title.pack(fill='x')

        self.shapes_label = tk.Label(
            root, text="", font=("Arial", 36, "bold"), 
            bg="#F4F6F6", fg="#BA2604"
        )
        self.shapes_label.pack(pady=(20, 10))
        ...
        self.prev_btn = ttk.Button(nav_frame, text="⏮ Prev", command=self.prev_shape, style='Nav.TButton')
        self.prev_btn.grid(row=0, column=0, padx=40)

        self.next_btn = ttk.Button(nav_frame, text="Next ⏭", command=self.next_shape, style='Nav.TButton')
    ...
    def show_shapes(self):
        current_shape = shapes[self.index]
        self.shapes_label.config(text=current_shape)

        name = current_shape.lower()
        candidates = [
            name,
            name.replace(" ", "_"),
            name.replace(" ", "-"),
    ...
    def speak_shape(self):
        ...
        try:
            ...
            current_shapevoice = shapes[self.index]
            engine.say(f" {current_shapevoice}")
            engine.runAndWait()
        except Exception as e:
...
def run(parent_root, main_menu, show_frame):
    ...
    app = ShapesApp(toplevel, main_menu, show_frame)
