...
animals = [
    "Lion", "Tiger", "Cow", "Cat", "Dog", "Fox", "Leopard",
    "Deer", "Bear", "Goat", "Turtle", "Donkey", "Giraffe", "Hippopotams", "Monkey",
    "Snake", "Crocodile", "Elephant", "Wolf", "Camel", "Horse", "Rabbit",
    "Pig", "Panda", "Gorilla", "Zebra"
  
]



IMAGE_DIR = r"D:/vvd/images/animals"
...
class AnimalsApp:
    def __init__(self, root, main_menu, show_frame):
       ...
       #self.root.geometry("450x650")
        ...
        self.app_title = create_title_label(root, "Animals")
        self.app_title.pack(fill='x')

        self.veg_label = tk.Label(
            root, text="", font=("Arial", 36, "bold"), 
            bg="#F4F6F6", fg="#BA2604"
        )
        self.veg_label.pack(pady=(20, 10))
        ...
        self.prev_btn = ttk.Button(nav_frame, text="⏮ Prev", command=self.prev_veg, style='Nav.TButton')
        self.prev_btn.grid(row=0, column=0, padx=40)

        self.next_btn = ttk.Button(nav_frame, text="Next ⏭", command=self.next_veg, style='Nav.TButton')
    ...
    def show_animals(self):
        animal_name = animals[self.index]
        self.veg_label.config(text=animal_name)
    ...
    def speak_animal(self):
        ...
        try:
            ...
            current_animal = animals[self.index]
            engine.say(f"This is a {current_animal}")
            engine.runAndWait()
        except Exception as e:
...
def run(parent_root, main_menu, show_frame):
    ...
    app = AnimalsApp(toplevel, main_menu, show_frame)
